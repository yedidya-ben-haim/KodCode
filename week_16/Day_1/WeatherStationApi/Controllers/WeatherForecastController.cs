...
public class WeatherReadingsController : ControllerBase
{
    ...
    public ActionResult<IEnumerable<WeatherReading>> GetReadingsByLocation(
    string location)
    {
        var readings = _readings
            .Where(r => r.Location.Equals(
                location,
        ...
        [FromQuery] double? maxTemp)
    {
        ...
        {
            query = query.Where(
                r => r.TemperatureCelsius >= minTemp.Value);
        }

        if (maxTemp.HasValue)
        {
            query = query.Where(
                r => r.TemperatureCelsius <= maxTemp.Value);
        }
